...
/**
 * Auth adapter for Doctrine v 1.2
 *
 * @category        Extlib
 * @package         Extlib\Auth
 * @subpackage      Extlib\Auth\Adapter
 * @author          Jason Eisenmenger & David A. Werner (http://framework.zend.com/wiki/pages/viewpage.action?pageId=3866950)
 * @modification    Lukasz Ciolecki <ciolecki.lukasz@gmail.com>
 * @copyright       Copyright (c) Jason Eisenmenger & David A. Werner
 */
class Doctrine implements \Zend_Auth_Adapter_Interface
{
    ...
    /**
     * $_conditions - additional conditions 
     * 
     * @var array 
     */
     ...
     * 
     * @param \Doctrine_Connection_Common $conn
     * @return \Extlib\Auth\Adapter\Doctrine
     */
     ...
     * 
     * @return \Doctrine_Connection_Common
     */
    ...
    /**
     * getConditions() - get addionals conditions
     * 
     * @return array 
     */
    ...
    /**
     * setConditions() - set addionals conditions
     * 
    ...
    /**
     * addConditions() - method add next condition
     * 
     * @param array $add 
     * @return \Extlib\Auth\Adapter\Doctrine
     */
    ...
    {
        $this->_conditions = array_merge($this->_conditions, $add);
        return $this;
    }
    ...
    /**
     * getAuthObject - method return auth user object with all connections between tables or 
     * false when search object doesn't exsist
     *
     * @return boolean || $this->_tableName
     */
    public function getAuthObject()
    {
        ...
        if (null === $this->_tableName || null === $this->_identityColumn || null === $this->_identity) {
            return false;
        }

        $dbSelect = \Doctrine_Query::create()
                ->from("$this->_tableName table")
                ->where("table.$this->_identityColumn = ?", $this->_identity);

        foreach ($this->_conditions as $condition => $value) {
            $dbSelect->addWhere($condition, $value);
        }

        $returnObject = $dbSelect->fetchOne();
    ...
    protected function _authenticateSetup()
    {
        ...
        if ($this->getConnection() === null) {
            ...
            $exception = 'A table must be supplied for the \Extlib\Auth\Adapter\Doctrine authentication adapter.';
        } elseif ($this->_identityColumn == '') {
            $exception = 'An identity column must be supplied for the \Extlib\Auth\Adapter\Doctrine authentication adapter.';
        } elseif ($this->_credentialColumn == '') {
            $exception = 'A credential column must be supplied for the \Extlib\Auth\Adapter\Doctrine authentication adapter.';
        } elseif ($this->_identity == '') {
            $exception = 'A value for the identity was not provided prior to authentication with \Extlib\Auth\Adapter\Doctrine.';
        } elseif ($this->_credential === null) {
            $exception = 'A credential value was not provided prior to authentication with \Extlib\Auth\Adapter\Doctrine.';
        }
